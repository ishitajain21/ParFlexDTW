...
# Minimal example (uncomment and set paths):
# F1 = np.load("path/to/features1.npy", allow_pickle=True)
# F2 = np.load("path/to/features2.npy", allow_pickle=True)
# C, tiled_result = align_system_sparse_parflex(F1, F2, steps=..., weights=..., beta=0.1)
# stage2 = sparse_parflex_2a(tiled_result, C, beta=0.1, show_fig=False)
# best_cost, wp = stage2["best_cost"], stage2["stitched_wp"]
# To plot: run global FlexDTW for flex_wp, then plot_parflex_with_chunk_S_background(tiled_result, C, flex_wp, stage2)


# In[ ]:


# Optional: full comparison (Parflex vs Sparse Parflex) and plot – load F1, F2 and run from run_sparse_parflex_L_sweep or 04_Align_Benchmarks.
