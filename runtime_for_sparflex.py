...
def parflex(C, steps, weights, beta, L=None, profile_dir='Profiling_results'):
    """
    Run Sparse Parflex on cost matrix C. Returns (best_cost, wp) with wp shape (2, N).
    ...
    profile_dir : str or None
        ...
        in a loop (e.g. profile_dir='runs/sparse_P1000_L500_trial3').
...
# The block below is for interactive/notebook testing only.
# It is guarded so that importing this module as a library does not execute it.
if __name__ == '__main__':
    ...
    steps = {
        'dtw1': np.array([1,1,1,2,2,1]).reshape((-1,2)),
